...
export const storeBeastsToFile = async (beasts: any[]): Promise<void> => {
    try {
        if (isWeb) {
            ...
            // 4. Save index to localStorage with error handling
            const indexJson = JSON.stringify(indexData);
            const indexSuccess = safeSetItem(WEB_STORAGE_KEYS.BEASTS_INDEX, indexJson);
            if (!indexSuccess) {
                throw new Error('Failed to save beasts index - storage quota exceeded');
            }
            console.log(`Stored index with ${allResolvedMonsters.length} monsters`);

            // 5. Save individual monster files to localStorage with error handling
            let savedCount = 0;
            for (const beast of allResolvedMonsters) {
                ...
                const beastJson = JSON.stringify(beast);
                const success = safeSetItem(key, beastJson);
                if (success) {
                    savedCount++;
                } else {
                    console.warn(`Failed to save monster ${beast.name} - storage quota exceeded`);
                    break; // Stop trying to save more if we hit quota
                }
            }

            console.log(`Stored ${savedCount} individual monster files (${allResolvedMonsters.length - savedCount} failed due to storage quota)`);
        } else {
...
export const storeSpellsToFile = async (spells: any[]): Promise<void> => {
    try {
        if (isWeb) {
            ...
            // Save index to localStorage with error handling
            const indexJson = JSON.stringify(indexData);
            const indexSuccess = safeSetItem(WEB_STORAGE_KEYS.SPELLS_INDEX, indexJson);
            if (!indexSuccess) {
                throw new Error('Failed to save spells index - storage quota exceeded');
            }
            console.log(`Stored index with ${spells.length} spells`);
            
            // Save individual spell files to localStorage with error handling
            let savedCount = 0;
            for (const spell of spells) {
                ...
                const success = safeSetItem(key, spellJson);
                if (success) {
                    savedCount++;
                } else {
                    console.warn(`Failed to save spell ${spell.name} - storage quota exceeded`);
                    break; // Stop trying to save more if we hit quota
                }
            }
            
            console.log(`Stored ${savedCount} individual spell files (${spells.length - savedCount} failed due to storage quota)`);
        } else {
            // Mobile implementation using expo-file-system
            await ensureDataDir();
...
export const storeCombatToFile = async (combat: any): Promise<void> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            console.log('Storing combat to localStorage:', combat.name, combat.id);
            const key = `${WEB_STORAGE_KEYS.COMBATS_PREFIX}${combat.id}`;
            const combatJson = JSON.stringify(combat);
            const success = safeSetItem(key, combatJson);
            if (success) {
                console.log('Combat stored with key:', key);
            } else {
                throw new Error('Failed to save combat - storage quota exceeded');
            }
        } else {
            // Mobile implementation using expo-file-system
            await ensureDataDir();
...
export const loadBeastsIndexFromFile = async (): Promise<any[] | null> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            const jsonData = safeGetItem(WEB_STORAGE_KEYS.BEASTS_INDEX);
            if (!jsonData) {
                return null;
            }
...
export const loadSpellsIndexFromFile = async (): Promise<any[] | null> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            const jsonData = safeGetItem(WEB_STORAGE_KEYS.SPELLS_INDEX);
            if (!jsonData) {
                return null;
            }
...
export const loadCombatsIndexFromFile = async (): Promise<any[] | null> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            console.log('Loading all combats from localStorage...');
            const combats: any[] = [];
            
            // Get all localStorage keys that start with the combats prefix
            console.log('Searching for combat keys in localStorage...');
            for (let i = 0; i < localStorage.length; i++) {
                const key = localStorage.key(i);
                console.log(`Checking key: ${key}`);
                if (key && key.startsWith(WEB_STORAGE_KEYS.COMBATS_PREFIX)) {
                    console.log(`Found combat key: ${key}`);
                    const combatData = safeGetItem(key);
                    if (combatData) {
                        try {
                            const combat = JSON.parse(combatData);
                            console.log('Found combat:', combat.name, combat.id);
                            console.log('Combat data:', combat);
                            console.log('Combatants count:', combat.combatants?.length || 0);
                            combats.push(combat);
                        } catch (e) {
                            console.error('Error parsing combat data for key:', key, e);
                        }
                    }
                }
            }
            ...
            console.log('Total combats loaded:', combats.length);
            return combats;
        } else {
            // Mobile implementation using expo-file-system
            console.log('Loading combats from file system...');
            await ensureDataDir();
            
            const combats: any[] = [];
            const files = await FileSystem.readDirectoryAsync(COMBATS_DIR);
            ...
            for (const filename of files) {
                if (filename.endsWith('.json')) {
                    try {
                        ...
                        console.error('Error reading combat file:', filename, e);
                    }
                }
            }
...
export const loadMonsterFromFile = async (filename: string): Promise<any | null> => {
    try {
        if (isWeb) {
            ...
            const jsonData = safeGetItem(key);
            if (!jsonData) {
                return null;
            }
...
export const deleteCombatFile = async (id: string): Promise<void> => {
    try {
        if (isWeb) {
            ...
            if (success) {
                console.log('Deleted combat with key:', key);
            } else {
                console.warn('Failed to delete combat with key:', key);
            }
        } else {
            // Mobile implementation using expo-file-system
            await ensureDataDir();
                ...
                if (filename.endsWith('.json')) {
                    try {
                        ...
                        if (combat.id === id) {
                            ...
                            console.log('Deleted combat file:', filename);
                            break;
                        }
                    } catch (e) {
                        console.error('Error reading combat file:', filename, e);
...
export const getStorageInfo = async (): Promise<{ 
    ...
    try {
        if (isWeb) {
            // Web implementation using localStorage
            const beastsIndexJson = safeGetItem(WEB_STORAGE_KEYS.BEASTS_INDEX);
            const spellsIndexJson = safeGetItem(WEB_STORAGE_KEYS.SPELLS_INDEX);
...
export const savePlayersList = async (players: any[]): Promise<void> => {
    try {
        if (isWeb) {
            ...
            const success = safeSetItem(WEB_STORAGE_KEYS.PLAYERS, playersJson);
            if (!success) {
                throw new Error('Failed to save players list - storage quota exceeded');
            }
        } else {
            // Mobile implementation using expo-file-system
            await ensureDataDir();
...
export const loadPlayersList = async (): Promise<any[]> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            const jsonData = safeGetItem(WEB_STORAGE_KEYS.PLAYERS);
            if (!jsonData) return [];
...
// Add a player
export const addPlayer = async (player: { name: string, race: string, class: string, maxHp: number, ac: number, passivePerception?: number, tokenUrl?: string }): Promise<void> => {
...
export const updatePlayer = async (name: string, updated: { name?: string, race?: string, class?: string, maxHp?: number, ac?: number, passivePerception?: number, tokenUrl?: string }): Promise<void> => {
    let players = await loadPlayersList();
...
export const loadSpellbooksFromFile = async (): Promise<any[]> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            const jsonData = safeGetItem(WEB_STORAGE_KEYS.SPELLBOOKS);
            if (!jsonData) return [];
            return JSON.parse(jsonData);
        } else {
            // Mobile implementation using expo-file-system
...
export const saveSpellbooksToFile = async (spellbooks: any[]): Promise<void> => {
    try {
        if (isWeb) {
            // Web implementation using localStorage
            const spellbooksJson = JSON.stringify(spellbooks);
            const success = safeSetItem(WEB_STORAGE_KEYS.SPELLBOOKS, spellbooksJson);
            if (!success) {
                throw new Error('Failed to save spellbooks - storage quota exceeded');
            }
        } else {
            // Mobile implementation using expo-file-system
            await ensureDataDir();
