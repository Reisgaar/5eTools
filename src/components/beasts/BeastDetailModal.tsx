...
// UTILS
import { getCachedCreatureImages } from 'src/utils/imageManager';
...
import { SpellNotFoundModal, SourceSelectionModal } from 'src/components/spells/modals';
import { CreatureNotFoundModal } from 'src/components/beasts/modals';
...
function formatSenses(senses: any, passive: any) {
    ...
    if (passive)
        s += (s ? ', ' : '') + `Passive Perception ${passive}`;
...
export default function BeastDetailModal({ visible, beast, onClose, theme, onCreaturePress, onSpellPress }: BeastDetailModalProps) {
    ...
    React.useEffect(() => {
        if (beast && beast['source'] && beast['name']) {
            ...
            if (lastBeastRef.current === beastKey) return;

            // Prevent multiple simultaneous loads
            if (loadingRef.current === beastKey) return;
    ...
    return (
        <>
            <Modal visible={visible} animationType="slide" transparent>
                <View style={[styles.modalOverlay, { paddingHorizontal: 10, paddingVertical: 30 }]}>
                    <View style={[styles.beastDetailModalContent, { flex: 1 }]}>
                                ...
                                <>
                                    <View style={styles.beastDetailBody}>
                                        <ScrollView contentContainerStyle={{ paddingBottom: 20 }} >
                                            <View style={[styles.beastDetailContent, { flex: 1 }]}>
